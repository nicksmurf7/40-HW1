...
int main(int argc, char *argv[]) 
{
    ...
    if (argc == 1) {
        ...
        Table_T matches = Table_new(sizeof(allfiles) * 10, NULL, NULL);
...
void compare_Files(List_T allfiles, Table_T matches) 
{
    ...
    while (parse != NULL) {
        ...
        while (compare_parse != NULL) {
            if (current == NULL || compare_parse == NULL){
                printf("Ligma""\n");
            }
        ...
        Table_T matches) 
{
    ...
    //read_file(Ffile, &line);
    int linecount = 0;
    int loopcheck1 = 0;
    int loopcheck2 = 0;
    while (read_file(Ffile, &line) != 1) {
        FILE *Fsecondaryfile = fopen(secondaryfile.filename, "r");
        loopcheck1++;
        linecount = linecount + 1;
        char* secondaryline;
        //read_file(Fsecondaryfile, &secondaryline);
        /* Call readaline on second file */
        //readaline(Fsecondaryfile, &secondaryline);
        while (read_file(Fsecondaryfile, &secondaryline) != 1) {
            loopcheck2++;
...
int read_file(FILE *file, char **line){
...
{
    //if (line == NULL || secondaryline == NULL){
    //    printf("LIGMA\n");
    //}
